...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit("Connecting to Ninja Userbot server and analysing the Image ...")
        try:
            ...
            await event.edit("Recognising Foreground and Background")
            output_file_name = ReTrieveFile(downloaded_file_name)
            os.remove(downloaded_file_name)
    elif input_str:
        await event.edit("Recognisation Complete!")
    ...
    if "image" in contentType:
        with io.BytesIO(output_file_name.content) as remove_bg_image:
            remove_bg_image.name = "Ninja Userbot.png"
        ...
        await event.edit("Removed Background in {} seconds, By Ninja Userbot".format(ms))
    else:
        await event.edit("ReMove.BG API returned Errors. Please report to @ninjanaveen\n`{}".format(output_file_name.content.decode("UTF-8")))
