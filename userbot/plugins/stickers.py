...
DEFAULTUSER = str(ALIVE_NAME) if ALIVE_NAME else "NinjaUserbot"
...
async def _(event):
    ...
    if userid == 1050176975:
        packname = f"NinjaNaveen_Stickers"
        packshortname = "Ninja_Stickers"
    else:
        packname = f"{DEFAULTUSER}'s NinjaUserbot Vol.{pack}"
        packshortname = f"NinjaUserBot_{userid}_kang"
    await event.edit("`Wow! What a Lovely Sticker! Mind if I Kang It?`")

    is_a_s = is_it_animated_sticker(reply_message)
    file_ext_ns_ion = "@NinjaBot.png"
    ...
    if is_a_s:
        ...
        if userid == 1050176975:
            packname = f"NinjaNaveenAnimated"
            packshortname = "NinjaNaveen_AnmStickers"
        else:
            packname = f"{DEFAULTUSER}'s NinjaUserbot Animated Vol.{pack}"
            packshortname = f"NinjaUserbot_{userid}" # format: Uni_Borg_userid
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    pack += 1
                    prevv = int(pack) - 1
                    packname = f"{DEFAULTUSER}'s Ninja Userbot Vol.{pack}"
